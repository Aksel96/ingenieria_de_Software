...
public class ComputadoraRestController {
    ...
    public ResponseEntity<Computadora> getComputadoraPorClave(@PathVariable Long clave) {
        ...
        //return new ResponseEntity<>(computadoraService.
        //      buscarPorId(clave).orElse(null),
        //    HttpStatus.OK);
    ...
    createComputadora(@RequestBody Computadora computadora) {
        return new
                ResponseEntity<>(computadoraService.crear(computadora), HttpStatus.CREATED);
    ...
    actualizaParcial(@PathVariable Long clave, @RequestBody Computadora computadora) {
        ...
        if (tmp != null) {
            ...
            // if else: se manda por ejemplo un campo demasiado grande
        } else {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
    }
